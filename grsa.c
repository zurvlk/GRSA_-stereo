...
int set_edge(Graph *G, int *ls, int *label, double T, int lambda, int label_max, int *left, int *right, img *raw_left, img *raw_right) {
    ...
    for (i = 1; i <= grids_node; i++) {
        ...
        for (j = 1; j < ls[0]; j++) {
            ...
            if(isin[i]) {
                ...
                if (dterm == 0) G->capa[edge_count] = D_function(ls[j], raw_left, raw_right, calheight(width, i), calwidth(width, i), label_max);
                else  G->capa[edge_count] = Dt_9n(ls[j], raw_left, raw_right, calheight(width, i), calwidth(width, i), label_max);
    ...
    for (i = 1; i <= grids_node; i++) {
        ...
        if(isin[i]) {
            ...
            if (dterm == 0) G->capa[edge_count] = D_function(ls[ls[0]], raw_left, raw_right, calheight(width, i), calwidth(width, i), label_max);
            else G->capa[edge_count] = Dt_9n(ls[ls[0]], raw_left, raw_right, calheight(width, i), calwidth(width, i), label_max);
